...
public class LinearizeGT {
  ...
  public static Node linearize2(Node node){ // 10 40  100
    if(node.children.size() == 0){
        return node; // 100
    }

    Node lastkiTail = linearize2(node.children.get(node.children.size() - 1)); // 40 100
    System.out.print(node.data+" ");
    while(node.children.size() > 1){
        Node lastChild = node.children.remove(node.children.size() - 1);
        Node secondLastChild = node.children.get(node.children.size() - 1);

        Node secondLastTail = linearize2(secondLastChild);
        secondLastTail.children.add(lastChild);
    }
    return lastkiTail;
  }
